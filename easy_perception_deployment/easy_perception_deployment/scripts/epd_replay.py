...
class Replay(Node):
    ...
    def _load_rgb(self, spec, width, height):
        ...
        image = cv2.imread(str((self.fixture_dir / spec["rgb"]).resolve()), cv2.IMREAD_COLOR)
        if image is None:
            raise ValueError("unable to read RGB fixture asset: " + spec["rgb"])
        if spec.get("crop_center") and image.shape[0] >= height and image.shape[1] >= width:
            top = (image.shape[0] - height) // 2
            left = (image.shape[1] - width) // 2
            return image[top:top + height, left:left + width].copy()
        return cv2.resize(image, (width, height)) if image.shape[:2] != (height, width) else image

    def _load_depth(self, spec, width, height):
        if "depth" in spec:
            depth = cv2.imread(
                str((self.fixture_dir / spec["depth"]).resolve()), cv2.IMREAD_UNCHANGED)
            if depth is None:
                raise ValueError("unable to read depth fixture asset: " + spec["depth"])
            if spec.get("crop_center") and depth.shape[0] >= height and depth.shape[1] >= width:
    ...
    def _messages(self, spec):
        ...
        rgb = self.bridge.cv2_to_imgmsg(self._load_rgb(spec, width, height), encoding="bgr8")
        depth = self.bridge.cv2_to_imgmsg(self._load_depth(spec, width, height), encoding="16UC1")
    ...
    def run(self):
        ...
        accepted = 0
        for index, spec in enumerate(observations, 1):
            ...
            # Exact synchronization truth: one ordered triplet with one source stamp.
            ...
            for _attempt in range(5):
                ...
                if self._spin_until(
                        lambda: self._number(self.metrics, "latest_observation_id") > baseline_id,
                        3.0):
                    delivered = True
                    break
            if not delivered:
                return self._finish(False, "observation was not accepted", duplicate_or_regressed)
            accepted = self._number(self.metrics, "latest_observation_id")
            if spec.get("wait_for_completion", True):
                if not self._spin_until(
                        lambda: self._number(self.metrics, "last_completed_observation_id") >= accepted or
                        self._number(self.metrics, "inference_failed") > 0,
                        float(self.fixture.get("inference_timeout_s", 60.0))):
                    return self._finish(False, "inference did not complete", duplicate_or_regressed)
                if self._number(self.metrics, "last_completed_observation_id") < accepted:
                    return self._finish(False, "production inference failed", duplicate_or_regressed)
        self._spin_until(
            lambda: self._number(self.metrics, "worker_busy") == 0 and
            self._number(self.metrics, "backlog_size") == 0, 60.0)
        return self._finish(accepted == len(observations), "", duplicate_or_regressed)
    ...
    def _finish(self, passed, failure="", duplicate_or_regressed=0):
        ...
        if metric("result_store_regressions") or metric("duplicate_result_publish"):
        ...
        summary = {
            ...
            "first_source_stamp_ns": int(observations[0]["stamp_ns"]) if observations else None,
            "last_source_stamp_ns": int(observations[-1]["stamp_ns"]) if observations else None,
            "first_observation_id": 1 if metric("latest_observation_id") else None,
...
def main():
    ...
    try:
        node = Replay()
        code = node.run()
    except Exception as error:  # Ensure launch receives a useful non-zero status.
