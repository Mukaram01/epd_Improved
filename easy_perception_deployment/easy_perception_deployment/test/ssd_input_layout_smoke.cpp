...
std::string PATH_TO_ONNX_MODEL(PATH_TO_PACKAGE "/data/model/ssd_mobilenet_v1_12.onnx");
...
TEST(EPD_TestSuite, Test_SSDMobileNet_Rank4Input_Smoke)
{
  ...
  usecase_config_json["usecase_mode"] = 1;
  Json::Value temp_arr;
  temp_arr[0] = Json::Value("person");
  usecase_config_json["class_list"] = temp_arr;
  ...
  EPD::EPDContainer ortAgent;
  EXPECT_EQ(ortAgent.precision_level, unsigned(2));

  cv::Mat frame = cv::imread(PATH_TO_TEST_IMAGE, cv::IMREAD_COLOR);
  ASSERT_FALSE(frame.empty());

  ortAgent.setFrameDimension(frame.cols, frame.rows);
  EXPECT_NO_THROW(ortAgent.initORTSessionHandler());
